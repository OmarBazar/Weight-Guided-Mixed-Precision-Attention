...
torch::Tensor launch_dummy_cpp(torch::Tensor g_mat1cols, torch::Tensor g_mat1values, torch::Tensor g_mat2, int N, int M, int F, int K, int H) {
    ...
    auto g_out = torch::empty({1, H, N, M}, torch::TensorOptions().dtype(torch::kFloat).device(torch::kCUDA));

    // Call the CUDA function to perform the addition
    launch_dummy(g_mat1cols, g_mat1values, g_mat2, N, M, F, K, H, g_out);
